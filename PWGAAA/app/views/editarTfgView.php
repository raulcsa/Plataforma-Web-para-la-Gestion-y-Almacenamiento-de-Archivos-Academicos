...
<body class="min-h-screen bg-gradient-to-br from-indigo-50 to-indigo-100 flex flex-col">
  ...
  <main class="flex-grow flex items-center justify-center p-6 bg-indigo-50">
  <div class="w-full max-w-2xl bg-white rounded-2xl shadow-xl p-8 transition-all duration-300 animate-fade-in">
    <h1 class="text-3xl font-bold text-indigo-600 mb-8 text-center">
      <?= $isEditor ? 'Editar TFG' : 'Detalle del TFG' ?>
    </h1>

    <?php if ($isEditor): ?>
      <form action="editarTfg?id=<?= $tfg['id'] ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
    <?php endif; ?>

      <!-- Título -->
      <div class="group">
        <label class="block text-sm font-medium text-gray-700 mb-1 transition-all group-hover:text-indigo-600">Título</label>
        <?php if ($isEditor): ?>
      ...
      <!-- Integrantes -->
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Integrantes</label>
        <?php if ($isEditor): ?>
      ...
      <div>
        ...
        <?php if ($isEditor): ?>
          <input type="date" name="fecha" value="<?= htmlspecialchars($tfg['fecha']) ?>"
                 class="px-4 py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 transition">
        <?php else: ?>
          <?php
            $fmt = new IntlDateFormatter('es-ES', IntlDateFormatter::LONG, IntlDateFormatter::NONE, 'Europe/Madrid', IntlDateFormatter::GREGORIAN);
      ...
      <div>
        ...
        <?php else: ?>
          <p class="text-gray-800 whitespace-pre-line"><?= nl2br(htmlspecialchars($tfg['resumen'])) ?></p>
        <?php endif; ?>
      </div>
      ...
      <div>
        ...
        <?php if ($isEditor): ?>
          <input type="file" name="pdf" accept=".pdf"
                 class="block w-full text-sm text-gray-600 border border-gray-300 rounded-md px-4 py-2 transition">
        ...
        <?php else: ?>
          ...
          <?php else: ?>
            <p class="text-gray-500 text-sm">No hay PDF subido</p>
          <?php endif; ?>
        <?php endif; ?>
      </div>
      ...
      <a href="proyectosPorCalificar"
           class="flex-1 text-center py-3 border border-gray-300 rounded-md hover:bg-gray-100 text-gray-700 transition hover:shadow-sm">
          Atrás
        </a>
        <button type="submit" name="action" value="calificar"
                class="flex-1 py-3 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-transform hover:scale-[1.02] shadow-md inline-flex items-center justify-center gap-2">
