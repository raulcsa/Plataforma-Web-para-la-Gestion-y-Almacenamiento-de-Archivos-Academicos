...
<body class="min-h-screen bg-gradient-to-br from-indigo-50 to-indigo-100 flex flex-col">
  ...
  <section class="text-center mb-10">
    <h1 class="text-3xl md:text-4xl font-extrabold text-indigo-700 animate-fade-in-up">Editar Usuario</h1>
    <p class="text-gray-600 mt-2 text-sm">Modifica los datos del usuario en TFCloud</p>
  ...
  <div class="mx-auto max-w-xl backdrop-blur-xl bg-white/70 border border-indigo-100 rounded-2xl shadow-xl p-8 animate-fade-in-up">
    <form method="POST" action="panelAdmin?action=update" class="space-y-5">
      <input type="hidden" name="id" value="<?= htmlspecialchars($usuario['id']); ?>">

      <div>
        <label for="nombre" class="block text-sm font-medium text-gray-700 mb-1">Nombre completo</label>
        <input type="text" name="nombre" id="nombre" value="<?= htmlspecialchars($usuario['nombre']); ?>" required class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500">
      </div>

      <div>
        <label for="email" class="block text-sm font-medium text-gray-700 mb-1">Correo electrónico</label>
        <input type="email" name="email" id="email" value="<?= htmlspecialchars($usuario['email']); ?>" required class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500">
      </div>
      ...
      <div>
        ...
        <input type="password" name="password" id="password" class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500">
      </div>
      ...
      <button type="submit" class="flex-1 py-1 bg-green-600 text-white rounded-md hover:bg-green-700 transition-all font-medium inline-flex items-center justify-center space-x-2">
