<?php
session_start();
// Aquí se asume que la variable $tfg y $archivos ya están definidas por el controlador
?>
<!DOCTYPE html>
<html lang="es">
<head>
  ...
  <!-- Puedes incluir también Google Fonts o estilos adicionales -->
  <style>
    ...
    .tfg-label { font-weight: bold; }
    .tfg-section { margin-bottom: 1rem; }
  </style>
</head>
<body>
  <!-- Navbar (mismo que en index) -->
  ...
  <div class="container mt-5">
    ...
    <!-- Sección para el fichero del TFG -->
    <?php if (!empty($archivos)): ?>
      <div class="tfg-section">
        <span class="tfg-label">Ficheros:</span>
        <ul>
          <?php foreach ($archivos as $archivo): ?>
            <li>
              <?php echo htmlspecialchars($archivo['nombre']); ?>
              (<a href="<?php echo htmlspecialchars($archivo['ruta']); ?>" target="_blank">Ver/Abrir</a>)
            </li>
          <?php endforeach; ?>
        </ul>
      </div>
